...
class FileManagerSkill(BaseSkill):
    ...
    def _execute(self, params: Dict[str, Any]) -> SkillResult:
        ...
        if not file_path:
            return SkillResult(
                success=False,
                message='No file path found in request. Please specify a file path.',
                data={'usage': "Try: 'read /path/to/file.txt' or 'show info about file.py'"}
            )
        try:
            ...
            if not file_path.exists():
                return SkillResult(
                    ...
                    data={'suggestions': ['Check if the file path is correct', 'Use absolute paths when possible', 'Ensure proper file permissions']}
                )
            if any((word in text for word in ['read', 'show', 'display', 'content'])):
                result = self._read_file(file_path)
                return SkillResult(success=True, message='File read successfully', data=result)
            elif any((word in text for word in ['info', 'information', 'details', 'size'])):
                result = self._file_info(file_path)
                return SkillResult(success=True, message='File info retrieved successfully', data=result)
            elif any((word in text for word in ['list', 'directory', 'ls', 'show files'])):
                result = self._list_directory(file_path)
                return SkillResult(success=True, message='Directory listed successfully', data=result)
            elif any((word in text for word in ['search', 'find', 'grep'])):
                search_term = self._extract_search_term(text)
                result = self._search_file(file_path, search_term)
                return SkillResult(success=True, message='File search completed successfully', data=result)
            else:
                result = self._file_info(file_path)
                return SkillResult(success=True, message='File info retrieved successfully', data=result)
            ...
            return SkillResult(
                ...
                data={'suggestion': 'Check file/directory permissions or try running with appropriate privileges'}
            )
