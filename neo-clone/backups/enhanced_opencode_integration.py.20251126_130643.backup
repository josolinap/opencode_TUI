...
class EnhancedOpencodeIntegration:
    ...
    def switch_model(self, target_model: str, reason: str = 'manual'):
        ...
        event = ModelSwitchingEvent(
            timestamp=time.time(),
            from_model=old_model,
            to_model=target_model,
            reason=reason,
            context_hash="",  # Will be populated when used in context
            performance_impact=0.0  # Will be calculated based on performance
        )
        self.model_switching_history.append(event)
        
        logger.info(f"Switched model from {old_model} to {target_model} (reason: {reason})")
        return True
    ...
    def record_request(self, user_input: str, model_used: str, response_time: float, success: bool):
