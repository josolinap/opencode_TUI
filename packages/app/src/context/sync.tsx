...
function init() {
  ...
  Promise.all(Object.values(load).map((p) => p())).then(() => setStore("ready", true)).catch((error) => {
    console.error("Failed to load API data:", error)
    // Set ready to true anyway so the UI shows, but with empty data
    setStore("ready", true)
  })
...
export function SyncProvider(props: ParentProps) {
  const value = init()
  return (
    <Show when={value.data.ready} fallback={
      <div class="h-full flex items-center justify-center bg-background text-text">
        <div class="text-center">
          <div class="text-2xl font-bold mb-4">OpenCode</div>
          <div class="text-text-muted mb-4">Loading...</div>
          <div class="text-sm text-text-muted/70">
            Make sure the API server is running on port 4096
          </div>
        </div>
      </div>
    }>
